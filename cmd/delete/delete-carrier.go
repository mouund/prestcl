...
// deletecarrierCmd represents the deletecarrier command
var deletecarrierCmd = &cobra.Command{
	Use:   "carrier",
	Short: "Delete a carrier from the prestashop instance",
	Long: ``,
	...
	Run: func(cmd *cobra.Command, args []string) {
		 ...
		 responseStatus = controllers.DeleteObjectFromId (encodedToken, shopUrl, "carriers", id)
		 if responseStatus == "200 OK" {
			fmt.Println("carrier deleted successfully")
		 }else{
...
func init() {
	DeleteCmd.AddCommand(deletecarrierCmd)

	 // definition of id flag
	 deletecarrierCmd.Flags().IntVarP(&id, "id", "i",0, "id of the resource to be deleted")
	 deletecarrierCmd.MarkFlagRequired("id")
}
